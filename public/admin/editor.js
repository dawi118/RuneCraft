...
const uploadOwner = () => activeTab==='tickets'&&ticketDraft ? `ticket:${ticketDraft.id}` : activeTab==='update'&&draft ? `update:${draft.id}` : activeTab==='settings'&&document.querySelector('[data-atlas-settings]') ? 'atlas' : 'library';
...
function renderUploadState() {
  ...
  document.querySelectorAll('#workspace-panel button[type=submit], [data-preview-ticket], [data-preview-update], [data-archive-ticket], [data-tab], [data-back-tickets], [data-back-updates], #sign-out, [data-refresh-media], [data-save-private], [data-new-update], [data-edit-update], [data-private-id], [data-edit-photo], [data-preview-settings], [data-manage-gallery], [data-content-tools], [data-feature-move], [data-feature-remove], [data-add-faq], [data-community-queue]').forEach(button=>button.disabled=uploader.blocked||publishing||button.dataset.featureMove==='0,-1');
  if(document.querySelector('#workspace-panel'))document.querySelector('#workspace-panel').inert=publishing;
  document.querySelectorAll('#workspace-panel input, #workspace-panel textarea, #workspace-panel select').forEach(input=>input.disabled=publishing||(input.matches('[data-feature-add]')&&uploader.blocked));
...
function saveLocal() {
  ...
  try { localStorage.setItem(key(), JSON.stringify({ base, content, draft, ticketDraft, activeTab, dirty, requestId, savedAt: new Date().toISOString() })); document.querySelector('[data-autosave]')?.replaceChildren(document.createTextNode(dirty ? 'Saved on this device. Click Save to website to publish.' : 'Published version loaded.')); }
...
async function loadWorkspace(keepCurrent = false) {
  ...
  render();
  if(keepCurrent&&activeTab==='update'&&(draft?.title||draft?.body))renderUpdate();
  ...
  if (saved?.dirty && !keepCurrent) {
    ...
    recovery.querySelector('[data-recover]').onclick=()=>{base=normalizeContent(saved.base);content=normalizeContent(saved.content);draft=saved.draft;ticketDraft=saved.ticketDraft;dirty=true;requestId=saved.requestId||crypto.randomUUID();activeTab=saved.activeTab|| (ticketDraft?'tickets':'update');ticketEditorOpen=activeTab==='tickets'&&!!ticketDraft;render(); if(activeTab==='update')renderUpdate();say('Local changes recovered. Click Save to website to publish.');};
...
function render() {
  ...
  const tabs = [['tickets','Build tickets'], ['places','Places'], ['update','Update archive'], ['settings','Site settings'], ['history','Version history']];
...
function renderUpdate() {
  draft ||= newDraft();
  panel().innerHTML=`<button type="button" data-back-updates>← Update archive</button><section class="admin-card"><h2>Recover private note</h2><form id="update-form" class="admin-form">${field('Title','title',draft.title)}${area('Note','body',draft.body)}<div class="actions"><button type="submit" data-save-private>Save privately</button><button type="button" data-export-note>Export note</button></div></form><div data-private-drafts></div></section>`;
  document.querySelector('[data-back-updates]').onclick=renderUpdates;
  document.querySelector('[data-export-note]').onclick=()=>download('gielinor-note.json',draft);
  const form=document.querySelector('#update-form');
  form.oninput=event=>{if(event.target.name)draft[event.target.name]=event.target.value;changed();};
  form.onsubmit=async event=>{event.preventDefault();saveLocal();try{await api('drafts','PUT',{id:draft.id,draft,content,baseRevision:base.revision});say('Note saved privately.');renderPrivateDrafts();}catch(error){handleError(error);}};
  renderPrivateDrafts();renderUploadState();
}
...
function renderPhotos() {
  ...
  document.querySelectorAll('[data-edit-photo], [data-preview-settings], [data-manage-gallery], [data-content-tools], [data-feature-move], [data-feature-remove], [data-add-faq], [data-community-queue]').forEach(button => button.onclick = () => {
...
function renderUpdates() {
  ...
  const list=()=>{const found=sortRecent(content.updates.filter(u=>u.state===updateState&&`${u.title} ${content.places.find(p=>p.id===u.placeId)?.name||''}`.toLowerCase().includes(updateQuery.toLowerCase())));
    document.querySelector('[data-update-results]').innerHTML=`<p>${found.length} updates</p>${found.slice(0,updateLimit).map(u=>`<article class="update-lookup-row"><div><h3>${e(u.title)}</h3><p>${e(content.places.find(p=>p.id===u.placeId)?.name||'')} · ${e(u.publishedAt?.slice(0,10)||'')}</p></div><div class="actions"><details><summary>Read update</summary><p>${e(u.body)}</p></details><button data-archive-note="${u.id}">${u.state==='archived'?'Restore update':'Archive update'}</button></div></article>`).join('')}${found.length>updateLimit?'<button data-more-updates>Show more updates</button>':''}`;
    ...
    document.querySelectorAll('[data-archive-note]').forEach(b=>b.onclick=async()=>{const proposed=clone(content),u=proposed.updates.find(u=>u.id===b.dataset.archiveNote);u.state=u.state==='archived'?'published':'archived';try{await savePublication(proposed,'/explore/');renderUpdates();}catch(error){handleError(error);}});
  };
  document.querySelector('[data-resume-update]')?.addEventListener('click',renderUpdate);
  document.querySelector('[data-export-updates]').onclick=()=>download('gielinor-updates.json',content.updates);
...
function renderSettings() {
  ...
  const form=document.querySelector('#settings-form');form.oninput=event=>{const input=event.target;if(!input.name)return;if(input.name.startsWith('faq-')){const[,kind,i]=input.name.split('-');s.faq[Number(i)][kind]=input.value;}else if(input.name.startsWith('nav-')){s.navLabels||={};s.navLabels[input.name.slice(4)]=input.value;}else if(input.name)s[input.name]=input.type==='checkbox'?input.checked:input.type==='number'?Number(input.value):input.value;changed();};
  form.elements.mapId.onchange=()=>{includeMedia(form.elements.mapId.value);document.querySelector('[data-map-preview]').innerHTML=img(media(s.mapId));};
  bindUploads(m=>{includeMedia(m.id);media(m.id).subject='Map';s.mapId=m.id;form.elements.mapId.innerHTML=options(mediaLibrary(),m.id,'alt');document.querySelector('[data-map-preview]').innerHTML=img(media(m.id));changed();});
  renderUploadState();

...
## Ticket workflow and standalone Atlas revision

The author’s follow-up makes places a canonical dictionary and build stages the editable tickets within those places. `normalizeContent` applies the same non-destructive conversion to the seed, live publications, publication baselines and restored versions. Around Lumbridge merges into Lumbridge; Lumbridge Castle and Lumbridge Swamp remain distinct. Ticket IDs and photographs are retained, references are remapped and old place URLs redirect through stored aliases. Duplicate names and alias conflicts are rejected. Place status is derived from its active tickets. New tickets receive server timestamps; unknown historical completion dates remain blank.

Explore now presents the same tickets in Full View and Build Board, with identical search, region, status and oldest/newest controls. Full View sorts by the latest recorded change (undated tickets last); the board groups Not Started, In Progress and Built. Gallery filters cascade region to place and sort photos by their recorded upload or associated content dates. Repeated joint-author bylines and the browser-only saved-place feature have been removed.

`/atlas/` is a primary navigation route with a large map viewport, pointer dragging, wheel and two-finger zoom, keyboard pan/zoom and a dismissible photo popup. Where a place has multiple tickets, its popup offers a ticket selector and direct links to the complete build pages. The preserved 2462px original map is served on this page instead of its smaller card derivative. Old Explore map links redirect to Atlas.

The author workspace opens on searchable Build tickets. A single ticket form handles title, canonical place lookup/creation, status, completion date, notes, scope, type, hours, uploaded/reused photographs, ordering, captions, focal points, optional place cover and map position. The Place dictionary supports lookup, creation, renaming, region changes and merging duplicates. Published updates has its own searchable archive/restore list. Gallery curation, imports/exports, external articles and optional community moderation are under Site settings; Version history remains available.

Save buttons explicitly publish to the website without deployment. Local autosave is a secondary recovery copy; it is never loaded over the published content without a choice. On focus, every 30 seconds, and atomically at save time, a newer server revision prompts the author to load the latest published version and discard local changes (with an export option). The author UI uses strict current-revision saves; lower-level merge support remains for historical compatibility and its existing integrity tests. Idempotent retries still recognize an already-committed save. Browser test writers run serially against their isolated store so tests do not themselves create conflicting author sessions.

Verification for this revision: 47 unit/integration tests and 65 enabled browser checks passed across five Chromium/WebKit profiles (40 duplicate publication cases intentionally skipped outside the single desktop writer). The final ticket recovery refinements were rebuilt and their five publication/recovery scenarios passed again. Phone testing includes native two-finger input through Chromium and touch-compatible controls in WebKit. These are browser profiles, not physical-device certifications.

## Atlas clarity and ticket carousel — 10 September 2026

- Replaced the popup ticket selector with a photo-led, scroll-snapping carousel. Previous/next buttons, ticket dots, mouse dragging, touch swiping and arrow keys work without automatic rotation. The selected ticket remains in the URL and restores on reload. Inactive slides are excluded from keyboard/screen-reader navigation; a single ticket needs no carousel controls.

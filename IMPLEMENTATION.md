...
Astro with the official Netlify adapter renders the public routes and metadata on request. Published content is not frozen during a code build. A shared content model powers rendering, author validation and the publication API. The browser receives small modules for navigation, the atlas, galleries, saved places and the editor. Fonts and responsive project photographs are local.

...
export async function GET({ site }) {
  ...
  const paths = ['/', '/explore/', '/progress/', '/journal/', '/gallery/', '/about/', '/community/', '/support/', '/credits/', '/privacy/', ...content.places.map(p => `/places/${p.slug}/`), ...content.stages.map(s => `/builds/${s.id}/`), ...content.updates.map(u => `/journal/${u.slug}/`), ...content.regions.filter(r => content.places.some(p => p.regionId === r.id)).map(r => `/regions/${r.id}/`), ...content.tours.filter(t => t.state === 'published').map(t => `/tours/${t.slug}/`)];

import { BRAND, escape as e, safeURL, formatDate, normalizeContent, canonicalStatus, sortRecent, recordDate, STATUSES } from './content.mjs';
...
export function picture(media, { hero = false, sizes = '(max-width: 650px) 100vw, (max-width: 1000px) 50vw, 33vw', cls = '' } = {}) {
  if (!media) return '<div class="missing-photo" role="img" aria-label="No photograph available"><img src="/brand/icon.webp" width="96" height="96" alt=""></div>';
...
export function placeCard(place, content, { compact = false } = {}) {
  ...
  return `<article class="place-card ${compact ? 'compact' : ''}">${link(`/places/${place.slug}/`, '', 'card-image').replace('</a>', `${picture(media(place.coverId))}</a>`)}<div class="card-copy"><h3>${link(`/places/${place.slug}/`, place.name)}</h3>${badge(place.status)}</div></article>`;
}
function updateCard(update, content) {
  const { media, place } = viewContext(content);
  const p = place(update.placeId);
  return `<article class="update-card"><a class="card-image" href="/journal/${e(update.slug)}/">${picture(media(update.mediaIds?.[0]))}</a><div><p class="eyebrow">${date(update.publishedAt)} · ${e(viewContext(content).place(update.placeId)?.name || '')}</p><h3>${link(`/journal/${update.slug}/`, update.title)}</h3><p class="meta">${p ? link(`/places/${p.slug}/`, p.name) : ''}</p></div></article>`;
}
function articleCard(article, content) {
  const { media } = viewContext(content);
  return `<article class="update-card"><a class="card-image" href="${e(safeURL(article.url))}">${picture(media(article.imageId))}</a><div><p class="eyebrow">${date(article.date)} · Substack</p><h3>${link(article.url, article.title)}</h3><p>${link(article.url, 'Read on Substack ↗', 'quiet-link')}</p></div></article>`;
}
...
function select(name, label, options, selected = '') { return `<label>${e(label)}<select name="${name}" aria-label="${e(label)}"><option value="">All ${e(label.toLowerCase())}</option>${options.map(option => { const [value, text] = Array.isArray(option) ? option : [option, option]; return `<option value="${e(value)}" ${selected === value ? 'selected' : ''}>${e(text)}</option>`; }).join('')}</select></label>`; }
...
function ticketCard(ticket, content) {
  ...
  return `<article class="ticket-card stage-row" data-ticket="${e(ticket.id)}"><a class="card-image" href="/builds/${e(ticket.id)}/">${picture(media(ticket.mediaIds[0]||p?.coverId))}</a><div class="card-copy"><p class="ticket-place">${p?link(`/places/${p.slug}/`,p.name):''}</p><h3>${link(`/builds/${ticket.id}/`,ticket.publicTitle)}</h3><p class="completion-date">Completion date: ${ticket.completedAt?date(ticket.completedAt):'Not recorded'}</p>${badge(ticket.status)}</div></article>`;
}
function atlasPopup(p, content, selected) {
  const tickets=sortRecent(content.stages.filter(s=>s.placeId===p.id && s.state!=='archived'));
  const ticket=tickets.find(s=>s.id===selected)||tickets[0],{media}=viewContext(content);
  return `<a class="atlas-close" href="/atlas/" aria-label="Close map popup">×</a><h2>${e(p.name)}</h2>${tickets.length>1?`<label>Build ticket<select data-popup-ticket>${tickets.map(t=>`<option value="${e(t.id)}" ${t===ticket?'selected':''}>${e(t.publicTitle)}</option>`).join('')}</select></label>`:''}${tickets.map(t=>`<div data-popup-build="${e(t.id)}" ${t===ticket?'':'hidden'}>${picture(media(t.mediaIds[0]||p.coverId))}<h3>${e(t.publicTitle)}</h3><p>${badge(t.status)} ${date(t.completedAt)}</p>${link(`/builds/${t.id}/`,'View complete ticket →','button brass')}</div>`).join('')||`${picture(media(p.coverId))}${link(`/places/${p.slug}/`,'View place →','button brass')}`}`;
}
export function renderPage(pathname, content, query = new URLSearchParams()) {
  ...
  if (path === '/') {
    ...
    const recent = latestArticle && (!updates[0] || latestArticle.date > updates[0].publishedAt) ? articleCard(latestArticle, content) : updates[0] ? updateCard(updates[0], content) : '';
    body = `<section class="home-hero">${picture(media(settings.heroId), { hero: true, sizes: '100vw', cls: 'hero-image' })}<div class="hero-shade"></div><div class="container hero-content"><h1><img src="/brand/logo.webp" width="1200" height="630" alt="Gielinor: Reforged" fetchpriority="high"></h1><p class="hero-purpose">RuneScape’s Gielinor, rebuilt in Minecraft.</p><div class="actions">${link('/explore/', 'Explore', 'button brass')}${link('/gallery/', 'Gallery', 'button ghost')}</div></div><div class="hero-caption container">${heroPlace ? link(`/places/${heroPlace.slug}/`, `${heroPlace.name} ↗`) : ''}</div></section>
    ...
    <section class="recent-section"><div class="container section"><div class="section-heading"><h2>Latest</h2>${link('/journal/', 'Journal →', 'quiet-link')}</div>${recent}</div></section>
    ...
    const matches = sortRecent(content.stages.filter(s => s.placeId && s.state !== 'archived' && (!query.get('q') || `${s.publicTitle} ${s.notes} ${place(s.placeId)?.name}`.toLowerCase().includes(query.get('q').toLowerCase())) && (!query.get('region') || place(s.placeId)?.regionId === query.get('region')) && (!query.get('status') || s.status === canonicalStatus(query.get('status')))), query.get('sort'));
    ...
    body = intro('', title, description).replace('page-intro container','page-intro container atlas-intro') + `<section class="atlas-shell" aria-label="Interactive atlas"><div class="map-controls" aria-label="Map controls"><button type="button" data-map-zoom="1.3" aria-label="Zoom map in">+</button><button type="button" data-map-zoom="0.76923" aria-label="Zoom map out">−</button><button type="button" data-map-reset>Reset</button></div><div class="map-viewport" tabindex="0" aria-label="Map of Gielinor. Drag to pan, scroll to zoom. Arrow keys pan; plus and minus zoom."><div class="map-canvas" data-focus-x="${chosen?.pin.x??.721}" data-focus-y="${chosen?.pin.y??.453}">${picture(media(settings.mapId)?.id==='00846e735be4772a'?{...media(settings.mapId),src:'/media/atlas-original.jpg',variants:{}}:media(settings.mapId),{hero:true,sizes:'100vw'})}${pins.map(p=>`<a class="map-pin" data-pin="${e(p.id)}" style="left:${p.pin.x*100}%;top:${p.pin.y*100}%" href="/atlas/?place=${e(p.id)}" aria-label="Select ${e(p.name)}"><img src="/brand/icon.webp" alt="" width="40" height="40"><span class="pin-label">${e(p.name)}</span></a>`).join('')}</div></div><aside class="atlas-popup" aria-label="Selected build" ${chosen?'':'hidden'} data-selected-place="${e(chosen?.id||'')}">${chosen?atlasPopup(chosen,content,query.get('ticket')):''}</aside>${pins.map(p=>`<template data-popup="${e(p.id)}">${atlasPopup(p,content)}</template>`).join('')}<noscript><p>Select a pin to view its build tickets. ${link('/explore/','Browse all tickets')}</p></noscript></section>`;
    ...
    body = intro(content.regions.find(r => r.id === p.regionId)?.name || 'Gielinor', title, description, `<div class="place-meta">${badge(p.status)}${history[0] ? `<span>${date(history[0].publishedAt)}</span>` : ''}</div>`) + `<div class="container">${placeGallery(images, p)}<div class="place-story section"><section><h2>Updates</h2>${history.length ? history.map(u => `<article class="history-entry"><p class="eyebrow">${date(u.publishedAt)} · ${e(u.kind || 'Build update')}</p><h3>${link(`/journal/${u.slug}/`, u.title)}</h3></article>`).join('') : ''}<details class="stage-details"><summary>Build tickets (${p.stageIds.length})</summary>${p.stageIds.map(id => content.stages.find(s => s.id === id)).filter(Boolean).map(s => stageRow(s, content)).join('')}</details></section><aside class="place-aside">${link(`/atlas/?place=${p.id}`, 'Map ↗')}${link(settings.instagram, 'Instagram ↗')}</aside></div><section class="section"><div class="section-heading"><h2>Nearby</h2></div><div class="place-grid">${(p.relatedIds || []).map(place).filter(Boolean).map(p => placeCard(p, content)).join('')}</div></section></div>${follow(settings)}`;
    ...
    const selection = settings.galleryMediaIds.map(media).filter(Boolean).map(m => {
      ...
      const stamps=[m.createdAt,...content.stages.filter(s=>s.mediaIds.includes(m.id)).map(recordDate),...updates.filter(u=>u.mediaIds.includes(m.id)).map(recordDate)].filter(Boolean).sort();
      return {id:m.id,m,p,updatedAt:stamps.at(-1)||''};
    ...
    body = intro('', title, description) + `<div class="container">${filterForm(select('region','Region',content.regions.map(r=>[r.id,r.name]),query.get('region'))+select('place','Place',availablePlaces.map(p=>[p.id,p.name]),selectedPlace)+sortSelect(query), '/gallery/', query)}<p class="results" role="status">${selection.length} photographs</p><section class="gallery-grid" aria-label="Project photographs">${sortRecent(selection,query.get('sort')).map(({m,p},index)=>photoLink(m,p,{index})).join('')||'<p class="empty-state">No photographs match.</p>'}</section></div>`;
  } else if (path === '/journal/') {
    title = 'Journal'; description = 'Build updates from Gielinor: Reforged.';
    body = intro('The journal', title, description, `<p class="intro-links">${link('/rss.xml', 'RSS')}${link('/progress/', 'Progress')}${link(settings.substack, 'Substack ↗')}</p>`) + `<div class="container section journal-list">${updates.map(u => updateCard(u, content)).join('') || ''}${content.articles.length ? `<section><h2>Substack</h2>${content.articles.map(a => articleCard(a, content)).join('')}</section>` : ''}</div>`;
    ...
    body = intro(u.kind || 'Build journal', title, '', `<p class="byline">${date(u.publishedAt)}${p ? ` · ${link(`/places/${p.slug}/`, p.name)}` : ''}</p>`) + `<article class="container reading journal-body">${richText(u.body)}${u.mediaIds.map(media).filter(Boolean).map(m => photoLink(m, p)).join('')}${u.stageId ? `<p>${link(`/builds/${u.stageId}/`, 'Read the original build stage →')}</p>` : ''}${p ? `<p>${link(`/places/${p.slug}/`, `Explore ${p.name} →`, 'button')}</p>` : ''}<p>${link('/journal/', 'Back to the journal')}</p></article>${follow(settings)}`;
    ...
    body = intro('Around Gielinor', title, description, `<p>${badge(region.status)} · ${places.length} places</p>`) + `<div class="container section"><div class="place-grid">${places.map(p => placeCard(p, content)).join('')}</div><section class="section"><h2>Recent work in ${e(region.name)}</h2>${updates.filter(u => places.some(p => p.id === u.placeId)).slice(0, 3).map(u => updateCard(u, content)).join('')}</section>${link(`/atlas/?region=${region.id}`, 'View on the atlas →')}</div>`;
  } else if (path === '/about/') {
